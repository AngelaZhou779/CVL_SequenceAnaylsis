...
for file in ${files[@]}
	...
	#Split length into 12 segements (12th == length) (can extend this if too large)
	cut=$((${length}/12))
	cut_2=$((${cut}*2))
	cut_3=$((${cut}*3))
	cut_4=$((${cut}*4))
	...
	for file in ${syncs[@]}
	  	...
	  	Rscript ${Rscripts}/poolSeq_selectionCoeff_DOWNandLAAD.R ${file} ${Chromo} ${split_sync}) &
	done 
	wait
