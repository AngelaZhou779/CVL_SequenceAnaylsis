...
### Read in the data file for args[1]
  ...
  mySync <- read.sync_Personal(file=args[1], gen=c(0,10,24,0,10,24,0,10,24), repl=c(1,1,1,2,2,2,3,3,3), polarization = "rising")
...
pbj <- matrix(NA,length(pst2), 3)
 ...
 for (i in 1:length(pst2)) {
    b_b <- pst2[i]
    TrajTEST <- af.traj(mySync, args[2], repl=c(1,2,3), pos=b_b)
    BfsfTEST <- estimateSH(TrajTEST, Ne=90, t=ccc, h=0.5, haploid = FALSE, simulate.p.value=TRUE)
